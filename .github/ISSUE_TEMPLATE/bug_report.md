---
name: Bug Report
about: Report a bug or unexpected behavior in Oscura
title: '[BUG] '
labels: bug, needs-triage
assignees: ''
---

## Bug Description

<!-- A clear and concise description of what the bug is. -->

## Steps to Reproduce

1. Load file with '...'
2. Run analysis '...'
3. Observe error '...'

## Expected Behavior

<!-- What you expected to happen. -->

## Actual Behavior

<!-- What actually happened. Include error messages if any. -->

## Environment

- **Oscura Version**: <!-- e.g., 0.1.1 -->
- **Python Version**: <!-- e.g., 3.12.1 -->
- **Operating System**: <!-- e.g., Ubuntu 22.04, macOS 14.2, Windows 11 -->
- **Installation Method**: <!-- uv / pip / conda -->

## Minimal Reproducible Example

```python
# Minimal code to reproduce the issue
import oscura

# Your code here
```

## Error Message / Traceback

```
Full error traceback here (if applicable)
```

## Input Data Details

<!-- If the bug involves loading or processing data -->

- **File format**: <!-- e.g., Tektronix WFM, CSV, VCD, etc. -->
- **File size**: <!-- e.g., 2.5 MB -->
- **Sample rate**: <!-- if applicable -->
- **Number of channels**: <!-- if applicable -->

## Additional Context

<!-- Any other relevant information about the problem. -->

## Possible Solution

<!-- If you have suggestions on how to fix the bug, describe them here. -->

## Checklist

- [ ] I have searched existing issues to ensure this is not a duplicate
- [ ] I have included a minimal reproducible example
- [ ] I have included the full error traceback
- [ ] I have specified my environment details
